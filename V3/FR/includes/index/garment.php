<section id="garment">
	<div class="container">
		<div class="row">
			<div class="col-xs-12 col-md-6 content">
				<h2 data-animated="0">L'habit <span></span></h2>

				<p data-animated="0">Heddoko est un habit de compression intelligent avec capteurs de mouvement intégrés qui détectent les mouvements du corps au complet en 3D. Il est concu pour les athletes et les enthusiastes de sport leur offrant un coaching virtuel. </p>

				<div class="col-xs-12">
					<div class="row">
						<div class="col-xs-12 col-sm-4" data-animated="0"><img src="images/icons/icon-sensors.svg" /> <p>Capteurs intégrés</p></div>
						<div class="col-xs-12 col-sm-4" data-animated="0"><img src="images/icons/icon-body.svg" /> <p>Saisie de données de tout le corps</p></div>
						<div class="col-xs-12 col-sm-4" data-animated="0"><img src="images/icons/icon-resist.svg" /> <p>Resiste aux plus durs entraînements</p></div>
					</div>
				</div>

				<p data-animated="0"><a href="/FR/product/#garment" class="btn btn-default">Pour en savoir plus clickez ici</a></p>
			</div>
			<div class="col-xs-12 col-md-6 image">
				<img src="images/index/garment.png" data-animated="4" >
			</div>
		</div>
	</div>
</section>